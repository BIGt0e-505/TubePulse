...
| `runCommunityPostsCron` | Hourly when `minute === 0`, only when `TUBEPULSE_ENABLE_COMMUNITY_POSTS` is enabled | Poll YouTube community posts as latest-channel-post state: first poll seeds latest only with no notification; later polls surface only a changed latest activity ID. V2 testing must also scope eligible channels with `TUBEPULSE_COMMUNITY_POST_CHANNEL_ALLOWLIST`. | Disabled: no KV reads/writes and no external calls. Enabled: reads `channels:active`, `channel:{id}:firstPollAt:posts`, posts, subscribers, profile/settings/override/state; writes latest post, first-poll marker, device state. | Disabled: none. Enabled: current wired path still uses YouTube Data API `activities.list`; isolated InnerTube latest-post helper exists but is not wired into polling yet. | Disabled: none. Enabled: sends community-post pushes only for changed latest posts; reuses one FCM OAuth token per cron run; can cleanup dead devices. | High | Payload construction is compatible with cron `sendFCMPush`; the compatibility fix was deployed with cron worker version `7b2900aa-6c8b-4116-a3e6-56d53d1004e1` on 2026-06-25. |
