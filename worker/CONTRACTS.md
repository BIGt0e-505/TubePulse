...
| `runCommunityPostsCron` | Hourly when `minute === 0` | Poll YouTube community posts as latest-channel-post state: first poll seeds latest only with no notification; later polls surface only a changed latest activity ID. | Reads `channels:active`, `channel:{id}:firstPollAt:posts`, posts, subscribers, profile/settings/override/state; writes latest post, first-poll marker, device state. | YouTube Data API `activities.list`; FCM OAuth/message send. | Sends community-post pushes only for changed latest posts; reuses one FCM OAuth token per cron run; can cleanup dead devices. | High | Payload construction is compatible with cron `sendFCMPush`; the compatibility fix was deployed with cron worker version `7b2900aa-6c8b-4116-a3e6-56d53d1004e1` on 2026-06-25. |
...
| `channel:{id}:recent:posts` | Shared | Latest community post array, currently empty or one item. | Cron writes latest-state community post data; API `/feed` reads. |
| `channel:{id}:firstPollAt:posts` | Shared cleanup, cron writer | ISO timestamp sentinel for community-post first-run guard. | Cron writes after seeding latest-only post state; API/cron dead-channel cleanup deletes it. |
