...
| `runCommunityPostsCron` | Hourly when `minute === 0`, only when `TUBEPULSE_ENABLE_COMMUNITY_POSTS` is enabled and `TUBEPULSE_COMMUNITY_POST_CHANNEL_ALLOWLIST` contains channel IDs | Poll allowlisted channels via YouTube InnerTube browse Posts tab as latest-channel-post state: first poll seeds latest only with no notification; same latest ID is a no-op/compact; no latest clears cached post state; changed latest ID with an existing cached post is treated as deletion/rollback reconciliation and does not notify because relative timestamps cannot prove forward order. | Disabled or empty allowlist: no KV reads/writes and no external calls. Enabled with allowlist: reads `channels:active`, then only allowlisted active channels' `channel:{id}:firstPollAt:posts`, posts, subscribers, profile/settings/override/state; writes latest post, first-poll marker, device state. Clearing or replacing cached posts removes only stale `post:*` IDs from subscriber `device:{id}:state:{channelId}.unwatched`; video IDs are preserved. | Disabled/empty allowlist: none. Enabled with allowlist: InnerTube `youtubei/v1/browse` only for allowlisted active channels; no YouTube Data API `activities.list` use for community posts. | Disabled/empty allowlist: none. Enabled with allowlist: sends community-post pushes only when there was no cached post and a latest post appears after the first-poll sentinel exists; changed latest IDs with existing cached posts are suppressed as conservative reconciliation; reuses one FCM OAuth token per cron run; can cleanup dead devices. | High | Payload construction is compatible with cron `sendFCMPush`; the compatibility fix was deployed with cron worker version `7b2900aa-6c8b-4116-a3e6-56d53d1004e1` on 2026-06-25. |
